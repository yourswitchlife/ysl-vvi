...
export function ShippingProvider({ children }) {
    ...
    const handleSelectChange = (e, memberId) => {
        ...
        const hasAddresses = addresses.homeAddresses.length > 0 || addresses.sevenAddresses.length > 0
        if (hasAddresses) {
    ...
    console.log("使用前的 shippingOptions:", shippingOptions);
    // 整理shippingInfos成收件人姓名、收件人電話、地址
    const getFormatShippingInfos = (shippingInfos, shippingOptions) => {
        console.log("使用後的 shippingOptions:", shippingOptions);
    ...
    console.log("收件資訊:", shippingInfos);
    ...
    useEffect(() => {
        let total = Object.values(shippingFees).reduce((total, fee) => total + fee, 0)
    ...
    const calcUpdateShippingFee = (memberId, selectAddrOption) => {
        let newFee = selectAddrOption === '1' ? 60 : selectAddrOption === '2' ? 100 : 0
    ...
    useEffect(() => {
        ...
        const newOrderGroup = cartItems.filter(item => item.userSelect === true)
            .reduce((group, item) => {
    ...
    useEffect(() => {
        ...
        Object.keys(orderGroup).forEach(memberId => {
            if (shippingOptions[memberId]?.selectAddrOption === '2' && addresses.homeAddresses.length > 0) {
                newSelectedAddresses[memberId] = { home: 0 }
            } else if (shippingOptions[memberId]?.selectAddrOption === '1' && addresses.sevenAddresses.length > 0) {
    ...
    useEffect(() => {
        Object.keys(orderGroup).forEach(memberId => {
            // 默認預設宅配
            const selectAddrOption = shippingOptions[memberId]?.selectAddrOption || '2'
            ...
            if (selectAddrOption === '2' && addresses.homeAddresses.length > 0) {
                ...
                newShippingInfo = {
                    ...
                    deliveryTime: defaultAddress.specialPreferences?.DeliveryTimePreference,
                }
            ...
            if (newShippingInfo) {
                setShippingInfos(currentInfos => ({
                    ...currentInfos, [memberId]: newShippingInfo
                }))
            }
        })
    ...
    useEffect(() => {
        ...
        const allOrdersHaveCompleteShippingInfo = pendingPaymentOrderIds.every(memberId => {
            const info = shippingInfos[memberId];
            // 检查是否存在收件信息，并且这些信息是否完整
            return info && info.name && info.phone && info.address;
        });

        if (!allOrdersHaveCompleteShippingInfo) {
            console.warn("请为所有订单填写完整的收件地址信息");
        } else {
            console.log("所有待支付的订单都已选择并填写了完整的收件地址信息");
        }
