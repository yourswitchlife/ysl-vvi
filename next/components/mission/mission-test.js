...
export default function MissionTest() {
        ...
        {mission.map(m => (
          <div key={m.id} className={`${styles.bg} m-2 col-md-6 col-sm-12`}>
            ...
            <div className='d-flex justify-content-center align-items-center mt-3 mb-3'>
              <button className={`btn ${m.status === 1 && solved ? (prize ? 'btn-dark' : 'btn-info') : 'btn-danger'}`}
                onClick={() => {
                  if (m.status === 1 && solved && !prize) {
                    claimPrize()
                  } else if (m.status === 0 || !solved) {
                    router.push('/products')
                  }
                }}

                disabled={prize}> {m.status === 1 && solved ? (prize ? '任務完成' : '快來領獎勵') : '去解任務'}</button>
            </div>
          </div>
        ))}
      </div>
