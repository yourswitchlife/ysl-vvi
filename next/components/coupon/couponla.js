...
export default function Coupons() {
  ...
  useEffect(() => {
    fetch(`http://localhost:3005/api/coupon/all?memberId=${memberId}`)
  ...
  const handleGet = (id) => {
    ...
    // const didClaimed = claimed.map(coupon => coupon.claimed === 1)
    const coupon = claimed.filter((coupon) => coupon.id === id && coupon.claimed === 1)
